...
> 
> func main() {
> 	nums := []int{1,2,4,3,3,5}
> 	fmt.Println(slideWindow(nums))
> }
> 
> func slideWindow(nums []int) (res int) {
> 	slideMap := make(map[int]int)
> 	res = 0
> 	for i, j := 0, 0; i < len(nums); i++ {
> 		// ok的话 说明访问成功 意味着nums[i]在之前的过程中已经插入map中
> 		for _, ok := slideMap[nums[i]];j < i && ok;  {
> 			//不断收紧左侧，直到去掉重复的nums[i]
> 			slideMap[nums[j]] -= 1
> 			j += 1
> 		}
> 		slideMap[nums[i]] += 1
